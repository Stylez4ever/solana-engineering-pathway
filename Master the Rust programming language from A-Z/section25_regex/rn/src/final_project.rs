...
fn main() {
    ...
    let re = Regex::new(r"\[(?<timestamp>\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})\]\s(?<catagory>INFO|ERROR|DEBUG|WARN):\s.+/api/(?<endpoint>\w+)/(?<id>\d+)").unwrap();
    ...
    let entries: Vec<_> = server_logs
        ...
        .map(|capture| LogEntry {
            ...
            category: capture["catagory"].to_string(),
